...
# Script developed by Jessica Blois
...
# I found another bug in the code! Ugh, sorry.
# I have also changed the next section of code to call a function.
# But, this bug does not appear to have affected the output data. I have checked the old and new datasets and they are the same! Whew.

source("code/samplesAnyChron_function.R")
datasetSamples <- samplesAnyChron(vertDownloads, "Syverson-Blois: bounds")
...
write.csv(dfs_unique_clean, file = "output/Neotoma_small_mammals_cleaned-v3.csv", row.names=F)

# Data check: compare with previous version
old_output <- read.csv(file = "output/Neotoma_small_mammals_cleaned-v2.csv", header=T)

samwell_new <- dfs_unique_clean %>% filter(collunitid == 10924) # corresponds to SCPD within Samwell Cave
samwell_old <- old_output %>% filter(collunitid == 10924)

# ok, the data are the same!
